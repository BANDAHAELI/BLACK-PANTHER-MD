...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^BLACKPANTHER_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^BLACKPANTHER_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^BLACKPANTHER_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/SARKAR;;;/gi, "").replace(/Vorterx;;;/gi, "").replace(/ibrahimaitech;;;/gi, "").trim();
